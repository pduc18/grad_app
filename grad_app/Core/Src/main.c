...
{
    char log_buffer[64];
    snprintf(log_buffer, sizeof(log_buffer), "X: %.2f, Y: %.2f\nTarget: %.2f, Current: %.2f\r\n PWM: %d\r\n ------------------ \r\n", x, y, target_position, current_position, getPWM());
    // Ensure the log buffer is null-terminated
    log_buffer[sizeof(log_buffer) - 1] = '\0'; // Safety null-termination
    // Send the log data over UART
